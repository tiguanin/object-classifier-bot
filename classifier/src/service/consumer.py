...
def consume():
    ...
    parameters = pika.ConnectionParameters(
        host=RMQ_HOST,
        port=RMQ_PORT,
        heartbeat=RMQ_HEARTBEAT_INTERVAL,
    ...
    channel.basic_consume(
        queue=RMQ_INPUT_QUEUE, on_message_callback=on_message, auto_ack=True
    )
...
def on_message(channel, method_frame, properties, body):
    ...
    try:
        ...
        LOGGER.error(
            "error during process {}, error: {}".format(body, traceback.format_exc())
        )
